...
class Permissions extends Model{
	...
	public function add($name, $name2,  $id_company){
		$sql = $this->db->prepare("INSERT INTO permission_params SET name=:name , name2=:name2 , id_company =:id_company");
		$sql->bindValue(':name', $name);
		$sql->bindValue(':name2', $name2);
		$sql->bindValue(':id_company', $id_company);
		$sql->execute();	
	}

	public function addGroup($name_group, $plist, $id_company){
		$params = implode(',', $plist);

		$sql = $this->db->prepare("INSERT INTO permission_groups SET name_group=:name_group , id_company =:id_company, params=:params");
		$sql->bindValue(':name_group', $name_group);
		$sql->bindValue(':id_company', $id_company);
	...
	public function editGroup($name, $plist, $id, $id_company){
		...
		$sql = $this->db->prepare("UPDATE permission_groups SET name_group=:name_group , id_company =:id_company, params=:params WHERE id=:id"  );
		$sql->bindValue(':name_group', $name);
		$sql->bindValue(':id_company', $id_company);
		$sql->bindValue(':params', $params);
		$sql->bindValue(':id', $id);
		$sql->execute();	
