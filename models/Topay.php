...
class Topay extends Model{
	...
	public function getList($offset, $id_company){
		...
		$sql = $this->db->prepare("SELECT * FROM topay WHERE id_company=:id_company ORDER BY  maturity DESC LIMIT $offset, 8 ");
	...
	public function getCount($id_company) {
		...
		$sql = $this->db->prepare("SELECT COUNT(*) as c FROM topay WHERE id_company = :id_company");
		$sql->bindValue(':id_company', $id_company);
		$sql->execute();
		$row = $sql->fetch();
	...
	public function add( $id_company, $id_user, $description, $maturity, $n_pmt, $pmt_n,  $price ){
		$id_provider='';
		$sql = $this->db->prepare("INSERT INTO topay SET id_company=:id_company, id_user=:id_user, description=:description , maturity=:maturity, n_pmt=:n_pmt, pmt_n=:pmt_n, id_provider=:id_provider, date_topay = NOW(), price=:price");	
		
		$sql->bindValue(":id_company",$id_company);
		$sql->bindValue(":id_user",$id_user);
		$sql->bindValue(":description",$description);
		$sql->bindValue(":maturity",$maturity);
		$sql->bindValue(":n_pmt",$n_pmt);
		$sql->bindValue(":pmt_n",$pmt_n);		
	...
	public function changeStatus($status, $id, $id_company) {
		...
		$sql = $this->db->prepare("UPDATE topay SET status = :status WHERE id = :id AND id_company = :id_company");
