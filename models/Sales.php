...
class Sales extends Model{
	...
	public function changeStatus($status, $id, $id_company) {
		...
		if($status == 1){
			...
			$sql = $this->db->prepare("SELECT * FROM sales WHERE  id = :id AND id_company = :id_company");
			$sql->bindValue(":id", $id);
			$sql->bindValue(":id_company", $id_company);
			$sql->execute();
			...
			$r->addRevenue( $id_company, $id_client, $id_user, $id, $total_price  );
		}

	}

	public function addSales($id_company, $id_client, $id_user, $quant, $status ){
		$i = new Inventory();
		...
		foreach ($quant as $id_prod => $quant_prod) {
			...
			if($sql->rowCount()>0){
				...
				$sqlp = $this->db->prepare("INSERT INTO sales_products SET id_company=:id_company , id_sale=:id_sale, id_product=:id_product, quant=:quant, sale_price=:sale_price, sale_date = NOW() ");
		...
		if($status==1){
			$r = new Revenue();
			$r->addRevenue( $id_company, $id_client, $id_user, $id_sale, $total_price );
		}
	...
	public function getSalesFistered($id_company, $client_name, $period1, $period2, $status, $order ){
    	$array = array();
    ...
    public function getSoldProducts($period1, $period2, $id_company) {
		...
		if($sql->rowCount() > 0) {
			...
			$sql = $this->db->query("SELECT SUM(quant) as total FROM sales_products WHERE id_sale IN (".implode(',', $p).")");
			$n = $sql->fetch();
			$int = $n['total'];
	...
	public function getProds($id_sale, $id_company){
		$array =array();
		$sql = $this->db->prepare("
			SELECT 
			...
			WHERE 
				sales_products.id_company=:id_company  AND sales_products.id_sale=:id_sale");
